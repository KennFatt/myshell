...
- `core/` holds the dependency manifest (`dependencies.sh`), shared paths, exports, aliases, and functions used everywhere.
- `dev/`, `git/`, and `pkg/` group topic-specific helpers.
- `scripts/` contains standalone utilities such as `json_to_block_comment.py`.
- `core/dependencies.sh` is the single source of truth for all external tool dependencies. Every `_bin` variable is declared there.
- `core/paths.sh` handles PATH setup only. Tool discovery lives in `dependencies.sh`.
...
- `myshell-doctor` verifies all external tools are installed and executable.
- `sh -n _init.sh core/*.sh dev/*.sh git/*.sh pkg/*.sh` checks shell syntax (includes subdirectories via glob).
...
- Every external command call should use its `_bin` variable declared in `core/dependencies.sh`, not a hardcoded binary name.
- Keep machine-specific values behind variables in `core/dependencies.sh` or exports, not hardcoded inside functions.
